<%@page import="Transferencias.ConsultaAfiliados"%>
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
    <%@ page import="Loguearse.Usuario"%>
    <%@ page import="java.util.LinkedList"%>
    <%@ page import="Transferencias.DirectorioCuentas" %>
    ...
    function validar()
{
    	...
    	if(combo.value.equals("")){
    		alert("Debe elegir una cuenta");
    		return false;
    	}
    	
    	if(document.directorio_trans_afiliados.combo.selectedIndex==0){
    		alert("Debe elegir una cuenta");
    		return false;
    	}
...
<form name="directorio"  action='./ConsultaTransf' method="post"  onsubmit="return validar()">

<select name="combo" id="combo">
<option value="">Seleccione una cuenta</option>
<%
	LinkedList<Usuario> lista = ConsultaAfiliados.Afiliado(objeto);
for (int i=0; i<lista.size(); i++)
{
	out.println("<option  value='"+lista.get(i).getCuentabene()+"'>"+lista.get(i).getCuentabene()+ "   "+lista.get(i).getAlias()+"</option>");
}
out.println("</select>");
%>
 <input type="hidden" name="cuenta" value ="<%= objeto.getCuenta()  %>">
 <input type="hidden" name="nombre" value ="<%= objeto.getNombre() %>">
<input type="submit" value="Aceptar" onclick="validar() " name="boton" />
   </form>
   <div> 
<jsp:include page="Datos_beneficiario.jsp" flush="true"/></div>
